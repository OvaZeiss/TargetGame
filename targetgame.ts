...
const TargetColor2: string = "white",
  TargetColor1: string = "red",
  BackgroundColor: string = "rgb(0,25,40)";
...
function CreateNewTarget(context: Context): Target {
  const myTarget: Target = new Target(
    //maxSize added in case random is 0
    Math.random() * 700 + maxSize,
    Math.random() * 500 + maxSize
  );
...
function main() {
  ...
  const displayElapsedTime = () => {
    ...
    const strMinutes: string =
      minutes < 10 ? "0" + minutes : JSON.stringify(minutes);

    const seconds: number = Math.floor((elapsedTime % 60000) / 1000);
    const strSeconds: string =
      seconds < 10 ? "0" + seconds : JSON.stringify(seconds % 100);
    const milisecondLength: number = JSON.stringify(
      Math.floor(elapsedTime)
    ).length;

    const milliseconds: string = JSON.stringify(Math.floor(elapsedTime)).slice(
      milisecondLength - 3,
      milisecondLength - 2
    );
    ...
    if (run) {
      document.querySelector(
        ".elapsedTime"
      ).textContent = `${strMinutes}:${strSeconds}:${milliseconds}`;
    }
  };
